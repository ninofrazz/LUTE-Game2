...
namespace LoGaCulture.LUTE
{
    ...
    public class InvokeMethodEditor : OrderEditor
    {
        ...
        private void ShowParameters(SerializedObject objTarget, GameObject gameObject, MethodInfo method)
        {
            ...
            if (objParams.Length > 0)
            {
                ...
                for (int i = 0; i < objParams.Length; i++)
                {
                    ...
                    if (isDrawn)
                    {
                        var vars = GetVariablesByType(targetMethod.GetEngine().Variables, objParam.ParameterType);
        ...
        private void ShowReturnValue(SerializedObject objTarget, MethodInfo method)
        {
            ...
            else if (method.ReturnType != typeof(void))
            {
                ...
                {
                    var vars = GetVariablesByType(targetMethod.GetEngine().Variables, method.ReturnType).ToList();
        ...
        private string[] GetVariablesByType(List<Variable> variables, Type type)
        {
